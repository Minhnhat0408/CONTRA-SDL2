...
MainCharacter::~MainCharacter(){
	...
	SDL_DestroyTexture(shooting.tex);shooting.tex =NULL;
	SDL_DestroyTexture(runshootup.tex);runshootup.tex = NULL;
	SDL_DestroyTexture(diving.tex);diving.tex = NULL;
	SDL_DestroyTexture(shootup.tex);shootup.tex = NULL;
	SDL_DestroyTexture(life_icon);life_icon = NULL;
				...
				{	
					if(!underwater || shootimg)
					{
						...
						{	
							...
							if(user_input.up == 1)
							{	
								SDL_Rect tmp = showClip;
								tmp.y -= 20;
								tmp.h +=20;
								SDL_RenderCopy(screen,shootup.tex,shootup_clip,&tmp);
							}else{
					...
					if( !underwater || shootimg)
					{
						...
						{	
							...
							{	
								...
								tmp.y -= 20;
								tmp.h +=20;
								SDL_RenderCopyEx(screen,shootup.tex,shootup_clip,&tmp,0,NULL,SDL_FLIP_HORIZONTAL);
							}else
							{
